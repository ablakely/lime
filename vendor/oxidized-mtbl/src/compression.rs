...
pub fn decompress(type_: CompressionType, data: &[u8]) -> io::Result<Cow<'_, [u8]>> {
    match type_ {
...
#[cfg(feature = "zlib")]
fn zlib_decompress(data: &[u8]) -> io::Result<Cow<'_, [u8]>> {
    use std::io::Read;
...
fn zlib_decompress(_data: &[u8]) -> io::Result<Cow<'_, [u8]>> {
    Err(io::Error::new(
        io::ErrorKind::Other,
        "unsupported zlib decompression",
    ))
}
...
#[cfg(feature = "snappy")]
fn snappy_decompress(data: &[u8]) -> io::Result<Cow<'_, [u8]>> {
...
fn snappy_decompress(_data: &[u8]) -> io::Result<Cow<'_, [u8]>> {
    Err(io::Error::new(
        io::ErrorKind::Other,
        "unsupported snappy decompression",
    ))
}
...
fn snappy_compress(_data: &[u8], _level: u32) -> io::Result<Cow<'_, [u8]>> {
    Err(io::Error::new(
        io::ErrorKind::Other,
        "unsupported snappy compression",
    ))
}
...
#[cfg(feature = "zstd")]
fn zstd_decompress(data: &[u8]) -> io::Result<Cow<'_, [u8]>> {
    let mut buffer = Vec::new();
...
fn zstd_decompress(_data: &[u8]) -> io::Result<Cow<'_, [u8]>> {
    Err(io::Error::new(
        io::ErrorKind::Other,
        "unsupported zstd decompression",
    ))
}
