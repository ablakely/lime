...
impl BlockCache {
    pub fn new(max_blocks: NonZeroUsize) -> Self {
        Self {
...
pub struct ReaderBuilder<'a> {
    ...
    block_cache: Option<&'a mut BlockCache>,
}

impl<'a> ReaderBuilder<'a> {
    pub fn new() -> ReaderBuilder<'a> {
        ReaderBuilder {
    ...
    pub fn block_cache(&mut self, cache: &'a mut BlockCache) -> &mut Self {
        self.block_cache = Some(cache);
        self
    }

    pub fn read<A: AsRef<[u8]>>(self, data: A) -> Result<Reader<'a, A>, Error> {
        ...
        Ok(Reader {
            ...
            verify_checksums,
            index,
            block_cache: self.block_cache.map(RefCell::new),
        })
    }
}
...
pub struct Reader<'a, A> {
    ...
    verify_checksums: bool,
    index: Rc<Block<A>>,
    block_cache: Option<RefCell<&'a mut BlockCache>>,
}

impl<'a, A> Reader<'a, A> {
    pub fn builder() -> ReaderBuilder<'static> {
        ReaderBuilder::new()
    }
}

impl<'a, A: AsRef<[u8]>> Reader<'a, A> {
    pub fn new(data: A) -> Result<Reader<'static, A>, Error> {
    ...
    pub fn into_iter(self) -> Result<ReaderIntoIter<'a, A>, Error> {
        ReaderIntoIter::new(self)
    }

    pub fn iter_from(self, start: &[u8]) -> Result<ReaderIntoIter<'a, A>, Error> {
        ReaderIntoIter::new_from(self, start)
    }

    pub fn iter_prefix(self, prefix: &[u8]) -> Result<ReaderIntoIter<'a, A>, Error> {
        ReaderIntoIter::new_get_prefix(self, prefix)
    }

    pub fn iter_range(self, start: &[u8], end: &[u8]) -> Result<ReaderIntoIter<'a, A>, Error> {
    ...
    fn block(&self, offset: usize) -> Result<Block<A>, Error> {
        ...
        if let Some(block_cache) = &self.block_cache {
            if let Some(bytes) = block_cache.borrow_mut().cache.get(&offset) {
                return Ok(Block::init(BytesView::from_bytes(bytes.clone()))
...
impl<'a, A: AsRef<[u8]>> ReaderIntoIter<'a, A> {
    fn new(r: Reader<A>) -> Result<ReaderIntoIter<A>, Error> {
        ...
        let bi = match r.block_at_index(&index_iter)? {
            Some(b) => {
                let mut bi = BlockIter::init(Rc::new(b));
                bi.seek_to_first();
                Some(bi)
            }
            None => None,
    ...
    fn new_from(r: Reader<'a, A>, key: &[u8]) -> Result<ReaderIntoIter<'a, A>, Error> {
        let mut index_iter = BlockIter::init(r.index.clone());
        index_iter.seek(key);
    ...
    fn new_get(r: Reader<'a, A>, key: &[u8]) -> Result<ReaderIntoIter<'a, A>, Error> {
    ...
    fn new_get_prefix(r: Reader<'a, A>, prefix: &[u8]) -> Result<ReaderIntoIter<'a, A>, Error> {
    ...
    ) -> Result<ReaderIntoIter<'a, A>, Error> {
    ...
    pub fn seek(&mut self, key: &[u8]) -> Result<bool, Error> {
        ...
        if self.block_offset != new_offset {
            ...
            self.bi = Some(BlockIter::init(Rc::new(b)));
        }
